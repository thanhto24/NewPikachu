...
void printBoard(int hei, int wid, int n, int m, int type, char view[200][200], char pic[200][200], bool movingOn[200][200], bool selected[200][200], char cpy[200][200])
{
    ...
    int lineColor = rand() % 15 + 1, basicColor = 7, picColor = (m + n) % 15 + 1, backgroundColor = 240, selecColor = 100;
    ...
    for (int i = 0; i <= n * hei - (n - 2); i++)
    {
        for (int j = 0; j <= m * wid - (m - 2); j++)
        {
            ...
            if (view[i][j] == '%')
            {
                SetConsoleTextAttribute(hConsole, hintColor);
                cout << view[i][j];
                SetConsoleTextAttribute(hConsole, basicColor);
                view[i][j] = '@';
                continue;
            }
            ...
            if (view[i][j] == '@' || view[i][j] == '$' || view[i][j] == '0'|| (view[i][j] >= 'A' && view[i][j] <= 'Z'))
            {
                ...
                {
                    if (view[i][j] == '@'||(view[i][j] >= 'A' && view[i][j] <= 'Z'))
                    {
                         SetConsoleTextAttribute(hConsole, selecColor);
                        if (view[i][j] >= 'A' && view[i][j] <= 'Z')
                            cout << view[i][j];
                        else
                            cout << ' ';
